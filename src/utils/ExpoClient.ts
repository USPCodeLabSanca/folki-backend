...
const requestRetryMinTimeout = 1000
...
export class Expo {
  ...
  constructor(options: ExpoClientOptions = {}) {
    ...
    this.limitConcurrentRequests = promiseLimit(
      options.maxConcurrentRequests != null ? options.maxConcurrentRequests : DEFAULT_CONCURRENT_REQUEST_LIMIT,
    )
  ...
  async getPushNotificationReceiptsAsync(receiptIds: ExpoPushReceiptId[]): Promise<{ [id: string]: ExpoPushReceipt }> {
  ...
  private async parseErrorResponseAsync(response: any): Promise<Error> {
    const textBody = await response.text()
    let result: ApiResult
    try {
      result = JSON.parse(textBody)
  ...
  private async getTextResponseErrorAsync(response: any, text: string): Promise<Error> {
    const apiError: ExtensibleError = new Error(
   ...
   */
  private getErrorFromResult(response: any, result: ApiResult): Error {
